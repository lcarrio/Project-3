...
unordered_map<string, Word> initializeUnorderedMap() {
...
map<string, Word> initializeOrderedMap() {
    ifstream reader1("demowords.txt");
    if (reader1.is_open()) {
        cout << "yes";
    }
...
int getValueUnordered(unordered_map<string, Word> dictionary, string userInput) {
    string word = "";
    double weight = 0.00;
...
int getValueOrdered(map<string, Word> dictionary, string userInput) {
    string word = "";
    double weight = 0.00;
...
public:
    ...
    {
        //The map time label is used to keep track of the time it takes to execute the map algorithms
        orderedMapTime.setText("Time for execution using map: ", juce::dontSendNotification);
        ...
        //The unordered map time label is used to keep track of the time it takes to execute the hash table algorithms
        ...
        inputText.onTextChange = [this] {
            ...
            //Initialize the two maps for comparison
            unordered_map<string, Word> dictionaryUnordered = initializeUnorderedMap();
            map<string, Word> dictionaryOrdered = initializeOrderedMap();

            //Get the value for the output
            int output = getValueUnordered(dictionaryUnordered, tweet);
            if (output == 0) {
            ...
            //Timer for comparing the two maps
            auto start = high_resolution_clock::now();
            getValueOrdered(dictionaryOrdered, tweet);
            auto stop = high_resolution_clock::now();
            auto duration = duration_cast<nanoseconds>(stop - start);
            int timeOrdered = duration.count();
            string timerO = to_string(timeOrdered);

            auto start2 = high_resolution_clock::now();
            getValueUnordered(dictionaryUnordered, tweet);
            auto stop2 = high_resolution_clock::now();
            auto durationUnordered = duration_cast<nanoseconds>(stop2 - start2);
            int timeUnordered = durationUnordered.count();
            string timerU = to_string(timeUnordered);

            juce::String mapTimeSeconds = "";
            juce::String hashTextSeconds = "";

            for (int i = 0; i < timerO.length(); i++) {
                char c = timerO[i];
                mapTimeSeconds += c;
            }
